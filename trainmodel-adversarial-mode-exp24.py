...
parser.add_argument('--print_every', type=int, default=100, help='frequency to print train loss, accuracy to terminal')
parser.add_argument('--save_every', type=int, default=4000000, help='frequency of saving the model')
...
parser.add_argument('--validate_every', type=int, default=1000, help='frequency of evaluating on validation set')
...
#---------------------------------------------------------------------------------------------------------------------------------
#generator_model = Generator(nstack = 8 , inp_dim = 256 , oup_dim = 14)

#discriminator_model_conf = Discriminator1(config['discriminator']['in_channels'], config['discriminator']['num_channels'], config['dataset']['num_joints'], config['discriminator']['num_residuals'])

#discriminator_model_pose = Discriminator(config['discriminator']['in_channels'] + 3 , config['discriminator']['num_channels'], config['dataset']['num_joints'], config['discriminator']['num_residuals'])
#---------------------------------------------------------------------------------------------------------------------------------


#--------------------------------------------------------------------
#pretrained Generator model#
modelpath_gen = torch.load('Adversarialmodel-pretrain-with-keepdimension(best)/model_58_10000.pt')
generator_model = modelpath_gen['generator_model']

#pretrained Discriminator model#
modelpath_disc = torch.load('Adversarialmodel-pretrain-with-keepdimension(best)/model_58_10000.pt')
discriminator_model_pose = modelpath_disc['discriminator_model_conf']
...
#
        ...
        outputs_d = [nn.Upsample(scale_factor= 256/64 , mode ='nearest')(outputs[-1].detach())]
        ...
        print("pose disc loss {}".format(loss_pose))
        ...
        print("pose time" , time.process_time() - start)
        ...
        loss = cur_gen_loss  + (cur_disc_loss_dic_pose)/180
        ...
        print("generator time" , time.process_time() - start)
        ...
        #print('type', type(cur_gen_loss))
        #print('type', type(cur_disc_loss))
        
        with open(os.path.join(args.modelName, 'stats.bin'), 'wb') as f:
            pickle.dump((disc_losses, gen_losses), f) ##disc_losses,, val_disc_losses, val_gen_losses
        ...
        print("end 1 time" , time.process_time() - start)
        if i % args.print_every == 0:
            print("Printing Images & Train iter: %d, generator loss : %f,  discriminator loss : %f" % (i ,gen_losses[-1], disc_losses[-1]))

            plt.clf()
            plt.imshow(np.sum(outputs[-1].detach().cpu().numpy()[0][0 : 14], axis=0))
            plt.savefig('trainingImages/train_output_exp24.png')
            
            
            plt.clf()
            plt.imshow(np.sum(ground_truth['occlusions'].detach().cpu().numpy()[0], axis=0))
            plt.savefig('trainingImages/train_gt_exp24.png')
            ...
            plt.savefig('trainingImages/train_img_exp24.png')
        # Save model
        ######################################################################################################################
        ...
        if i % args.validate_every == 0:
            for j, data in enumerate(val_eval_loader):
                ...
                with torch.no_grad():
                    outputs = generator_model(images)
                    outputs[-1] = outputs[-1][:, : config['dataset']['num_joints']]
                    eval_avg_acc, eval_cnt, _ = pck.StackedHourGlass(outputs, ground_truth['occlusions'])
...
#--------------------------------------------------------------------------------------------------------------------------#
        ...
        print("2 . end time" , time.process_time() - start)
