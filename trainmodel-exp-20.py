...
#from discriminator01 import Discriminator1
...
parser.add_argument('--save_every', type=int, default=40000, help='frequency of saving the model')
...
parser.add_argument('--train_split', type=float, default=0.9166)
...
#generator_model = Generator(nstack = 4 , inp_dim = 256 , oup_dim = 14)
...
#discriminator_model_pose = Discriminator1(config['discriminator']['in_channels'] + 3 , config['discriminator']['num_channels'], config['dataset']['num_joints'], config['discriminator']['num_residuals'])

#--------------------------------------------------------------------
modelpath = torch.load('LSP-keepdimension/LSP12000_VERSION02/model_229_1300.pt')
...
#        outputs = [outputs[-1]]
        ...
        #cur_gen_loss_dic = gen_single_loss(ground_truth, outputs, discriminator_model, mode=args.loss)
#        cur_disc_loss_dic = disc_loss_pose(ground_truth, outputs, images , discriminator_model_pose, detach=False )
...
#        print("generator real loss cur_disc_loss_dic_conf", cur_disc_loss_dic_conf)
#        print("generator real loss cur_disc_loss_dic_pose", cur_disc_loss_dic_pose)
#        cur_disc_loss =(cur_disc_loss_dic_pose)/180.+ (cur_disc_loss_dic_conf)/230.  

        loss = cur_gen_loss    #+ (cur_disc_loss_dic_pose)/180. + (cur_disc_loss_dic_conf)/230.
...
#        disc_losses.append(float(loss_conf))
        ...
        with open(os.path.join(args.modelName, 'stats.bin'), 'wb') as f:
            pickle.dump((gen_losses), f) ##disc_losses,, val_disc_losses, val_gen_losses
        ...
        if i % args.print_every == 0:
            print("Printing Images & Train iter: %d, generator loss : %f" % (i ,gen_losses[-1]))

            plt.clf()
            plt.imshow(np.sum(outputs[-1].detach().cpu().numpy()[0][0 : 14], axis=0))
            plt.savefig('trainingImages/train_output_exp20.png')
            
            
            plt.clf()
            plt.imshow(np.sum(ground_truth['occlusions'].detach().cpu().numpy()[0], axis=0))
            plt.savefig('trainingImages/train_gt_exp20.png')
            ...
            plt.savefig('trainingImages/train_img_exp20.png')
        # Save model
        ######################################################################################################################
        if i > 0 and i % 1300 == 0:
