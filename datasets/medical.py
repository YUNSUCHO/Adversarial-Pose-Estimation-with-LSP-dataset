...
parser.add_argument('--train_split', type=float, default=.99)
...
class HANDXRAY(Dataset):
    ...
    def _get_files(self):
        ...
        print("length of annotations",self.annot[1])
    ...
    def __getitem__(self, idx):
        ...
        heatmaps = np.zeros((len(annotations), 256, 256))
        ...
        for joint_id in range(len(annotations)):
            ...
            heatmaps[joint_id] = np.exp(-5*((x_c - xx)**2 + (y_c - yy)**2)/(10**2))
        ...
        return {
            ...
            'occlusions': torch.Tensor(heatmaps*200),
