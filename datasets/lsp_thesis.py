...
class LSP(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        crop_image_org = I.Crop(org, c, s1, 0, self.inputRes) / 255.
        ...
        heatmaps_org = np.zeros((14, 64, 64)) 
        ...
        for joint_id in range(annot.shape[0]):
            ...
            if annot[joint_id][0] >0: 
                ...
                annot_org= I.Transform(np.array([x_c1 , y_c1]), c, s1, 0, 64)
                ...
                heatmaps_org[joint_id] =I.DrawGaussian(heatmaps_org[joint_id], np.array([m_c1, n_c1]), 1, 0.5 if self.out_size==32 else -1)
        ...
        org = torch.Tensor(crop_image_org)
        #-------------------------------------------------------------
        ...
        return {
            ...
            'heatmaps': torch.Tensor(heatmaps_org*1.2),
...
if __name__ == '__main__':
    ...
    for i in range(len(dataset)): 
            ...
            plt.imshow((data['original'].numpy().transpose(1, 2, 0)*255).astype(np.uint8))
            plt.subplot(1, 4, 2)
