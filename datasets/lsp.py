...
parser.add_argument('--path', \
    default='/home/ycho/Adversarial-Pose-EstimationV.2/lspet_dataset')
...
class LSP(Dataset):
 ...
 #           occlusions[joint_id] =np.exp(-0.5*((m_c - mm)**2 + (n_c - nn)**2)/(self.occlusion_sigma**2))
        #---------------
        dis = np.linalg.norm(annot_b[3] - annot_b[8])
        #print("eludian distance: left shoulder and left hip" , dis)
        #---------------
        return {
            ...
            'occlusions': torch.Tensor(occlusions*1.6),
...
if __name__ == '__main__':
    ...
    for i in range(len(dataset)):
        ...
        plt.clf()
        print("eludian distance: left shoulder and left hip",data['distance'].numpy())
